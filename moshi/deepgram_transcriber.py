...
class DeepgramTranscriber:
    """
    Manages a SINGLE persistent Deepgram stream per connection using SDK v3.
    """
    def __init__(self, context_manager: ContextManager, 
        ...
        clog.log("info", "[Deepgram] DeepgramTranscriber initialized (SDK v3)")
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            # v3 SDK Initialization
            self.connection = self.client.listen.websocket.v("1")

            self.connection.on(LiveTranscriptionEvents.Open, lambda _, **kwargs: clog.log("info", "[Deepgram] Connection opened"))
            self.connection.on(LiveTranscriptionEvents.Close, lambda _, **kwargs: clog.log("info", "[Deepgram] Connection closed"))
            ...
            if self.connection.start(options):
                ...
                clog.log("error", "[Deepgram] Failed to start connection")
                self.enabled = False
    ...
    async def _keepalive_loop(self) -> None:
        ...
        while self.connection and self.enabled:
            ...
            try:
                # v3 uses .keep_alive() instead of .send_keep_alive()
                self.connection.keep_alive() 
            except Exception as e:
                clog.log("warning", f"[Deepgram] Keepalive warning: {e}")
                break
