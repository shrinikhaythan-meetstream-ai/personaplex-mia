...
class ContextManager:
    ...
    def __init__(self, developer_prompt: str = "", max_history: int = 15):
        ...
        # NOTE: current_partial removed — we now operate in final-transcript-only mode.
        self.max_history = max_history
        
    def update_transcript(self, transcript_json: Dict[str, Any]) -> None:
        """
        Process incoming transcript from Deepgram or model output.
        Only final transcripts are accepted; all partials are discarded
        immediately to keep the context clean.

        Args:
            transcript_json: Dict with keys:
                - 'speaker': Speaker label (e.g., "Speaker 0", "Moshi")
                - 'text': Transcribed/generated text
                - 'is_final': Boolean — MUST be True for storage
        """
        # Strictly ignore all partial/interim transcripts.
